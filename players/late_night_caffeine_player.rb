class LateNightCaffeinePlayer
  # Returns this players name.
  def name
    "Late Night Caffeine Player"
  end
...
#      (@letters_in_words_of_size_x ||= []) << letters.uniq
  ...
  def take_turn(pattern)
    ...
    bits.each do |bit|
    	next if bit.done?
        if bit.options.size < 4
	  guess = bit.options.reject{ |w| @used[w] }[0]
          @used[guess] = true
          return guess
	end
    ...
    # Do we nearly know the whole phrase?  If so return it
    if bits.all? { |bit| bit.done? || bit.options.size < 4 }
	bits.each { |bit|
		# Assume the guess waas right and use it to reduce the options
		bit.options = bit.options.select { |opt| opt.include? guess }
	}
    	return bits.map { |bit| "#{bit.options[0] || bit.pattern}" }.join("/")
    else
    	@used[guess] = true
  ...
  module U
    module_function
    def letters_in_words dict, sizes
       words = []
       sizes.sort.uniq.each do |size|
         words << dict[size]
       end
       words.map{|w|w.scan(/./)}.flatten.uniq
    end
  end
end
